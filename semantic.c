...
NOEUD create (const char* nom, TYPE_IDENTIFIER type, CLASS classs, NOEUD suivant){
...
NOEUD search (const char* nom, TABLE_NOUED table) {
    if( !table )
        return NULL;
    NOEUD noeud = table;
    while( noeud ){
...
void verifierVarID (char* nom){
    ...
    if (g_IfFonc){
        ...
        if(search(nom, table_local) ){
            semanticerror(concat("variable identifier already defined: ", nom));
        }else{
            NOEUD noeud = create(nom, g_type, classs, NULL);
        ...
        if(search(nom, table) ){
            ...
            NOEUD noeud = create(nom, g_type, attribute, NULL);
...
void verifierFoncID (char* nom){
         if( search(nom, table) ){
                                ...
                                g_noeudFonc = create(nom, g_type, fonction, NULL);
...
void verifierClassID (char* nom){
         if( search(nom, table_class) ){
                                semanticerror(concat("class identifier already defined: ", nom));
                            }else{
                                g_noeudClass = create(nom, NODE_TYPE_UNKNOWN, classs, NULL);
...
int verifierIDDeclare (char* nom){
    NOEUD noeud;
    if (g_IfFonc){
        noeud = search(nom, table_local);
        if ( !noeud ){
            noeud = search(nom, table);
            if( !noeud ){
